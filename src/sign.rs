...
pub struct OidcSigner {
    key: EncodingKey,
    pubkey: DecodingKey,
    /// the key ID used to identify the public key for this signer.
    pub kid: String,
    /// the key signing algorithim used.
    pub algorithm: Algorithm,
}

impl OidcSigner {
    /// create a token signer from RSA PEM string with the given key identifier.
    ...
    /// returns a validator that can be used to validate the authenticity of tokens generated by this signer. used when initializing self issue + authentication flows.
    pub fn validator(&self, issuer: &str, audience: &str) -> Result<Validator, OIDCError> {
        Validator::from_pubkey(
            issuer.to_string(),
            audience.to_string(),
            serde_json::to_string(&self.algorithm)?,
            self.pubkey.clone(),
        )
    }

    /// loads a PEM encoded file to produce tthe required information to build this signer.
